import { isNotDefined, head, last, isDefined, getClosestItemIndexes } from '../utils';
...
function extentsWrapper(useWholeData, clamp, pointsPerPxThreshold, minPointsPerPxThreshold, flipXScale) {
  console.log('[foutside] extentsWrapper');

  function filterData(data, inputDomain, xAccessor, initialXScale, option: {
    currentPlotData?: any, currentDomain?: any, fallbackStart?: any, fallbackEnd?: any
  } = null) {
    if (useWholeData) {
    ...
    if (filteredData.length === 1 && isDefined(option.fallbackStart)) {
      left = option.fallbackStart;
      right = getNewEnd(option.fallbackEnd, xAccessor, initialXScale, left);
    ...
    console.log(`Trying to show ${filteredData.length} points in ${width}px,`
    ...
    if (canShowTheseManyPeriods(width, filteredData.length, pointsPerPxThreshold, minPointsPerPxThreshold)) {
      ...
      if (chartWidth > showMaxThreshold(width, pointsPerPxThreshold) && isDefined(option.fallbackEnd)) {
        plotData = filteredData;
        const newEnd = getNewEnd(option.fallbackEnd, xAccessor, initialXScale, head(realInputDomain));
        domain = [
        ...
        console.log(`and ouch, that is too much, so instead showing ${plotData.length} in ${newWidth}px`);
      } else {
        plotData = option.currentPlotData || filteredData.slice(filteredData.length - showMax(width, pointsPerPxThreshold));
        domain = option.currentDomain || [xAccessor(head(plotData)), xAccessor(last(plotData))];
